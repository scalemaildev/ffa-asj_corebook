...
Throughout a chapter’s CYOA gameplay, the character may be presented with equipment and items. This gear can only be gathered once, even if the character makes repeated trips to the reading in which they are available. 
...
When the \refto{SP} or \refto{FP} cost of an action is appended by a ‘+’ sign, it can be overspent up to double the original cost.
...
The primary difference between these two options is that Light Attacks must be committed one-handed, while Heavy Attacks must be committed two-handed. Both of these attack types have their advantages and use cases.\\
In addition, there are other attacks that require the weapon to be either one or two-handed.\\
Weapons that are implicitly one-handed will not reach parity with two-handed weapons in terms of Heavy Attack damage output. However, two-handed weapons such as large swords have strict \refto{STR} requirements to be one-handed--and some two-handed melee weapons cannot be one-handed at all.
...
This conditional behavior only hinges on whether the enemy is within range for the attack, not whether the attack will actually damage its target.\\
...
Attack, Heavy & A & Wep+ & 2H M & Attack an enemy within weapon range.\newline Apply an extra point of damage for every overspent \refto{SP} up to double the weapon’s damage (including any scaling bonus damage)\\
...
Round Shield & 2 & 5 & 1 C & 2 & 1 & STR - 8 & N/A & Do not place blocked damage tokens from non-\reftoit{Magical} ranged attacks on \reftoit{Shield Up!}\\
...
Slink & DEX: 12 & 1 & 4 & F & Place a character on the opposite hex of an adjacent enemy \\
...
Fireball & - & 2 & 4 & 3 & Deal \refto{PWR} Burn ranged damage to a tile within 2-4 hexes, and \refto{PWR}/2 \emph{rd} Burn damage to all hexes adjacent to that tile\\
...
Flame-\newline touch & - & - & 2 & 2 & Deal 2 Burn damage to an adjacent enemy \\
...
Dazzle & INT: 12 & 2 & 4 & 3 & Inflict Stun on an enemy within \refto{PWR}*2 hexes, and at least 2 hexes away\\
